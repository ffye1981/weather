...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      var that = this
      this.$http.getData(
        'static/data/response_1000.json',
        { time: this.playTime, wkt: polygon },
        {},
        function(data, msg) {
    ...
    drawPoint() {
      ...
      function drawcircle(latlng, dir, speed) {
        ...
        // 绑定一个提示标签
        oMarker.bindTooltip('<p>' + dir + ',' + speed + '</p>', {
          direction: 'left',
          offset: [0, 0]
        })
      ...
      for (var i = 0; i < this.windData.length; i++) {
        drawcircle(
          [this.windData[i].lats, this.windData[i].lons],
          this.windData[i].direction,
          this.windData[i].speed
        )
      }
    }
  },
